...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("1’234.50", 1234.5),  # typographic apostrophe  # noqa: RUF001
        ("1ʼ234.50", 1234.5),  # modifier letter apostrophe (OCR)  # noqa: RUF001
...
@pytest.mark.parametrize(
    ...
    [
        ("42", 42),
        ("-7", -7),
        ("1'234", 1234),  # OCR apostrophe
        ("1 234", 1234),  # NBSP
        ("1,234", 1234),
        ("3.14", None),  # decimals are not ints
        ("", None),
        ("not a number", None),
    ],
)
def test_int_coercion(raw, expected):
    assert coerce_value(raw, "int") == expected
...
@pytest.mark.parametrize(
    "fname,spec,expected_type,expected_value",
    [
        ...
        (
            "amount",
            {"regex": r"Total à payer\s+EUR\s+([\d ,-]+)", "type": "float"},
            "float",
            1234.5,
        ),
        (
            "date",
            {"regex": r"Date d'émission\s+(\d{2}\.\d{2}\.\d{4})", "type": "date"},
            "date",
            "2024-03-15",
        ),
        (
            "invoice_number",
            [r"NotPresent\s+(\d+)", r"Numéro de facture\s+(\d+)"],
            "str",
            "987654321",
        ),
...
def test_invalid_regex_reports_error():
    f = extract_with_rule(
        ...
        make_rule(
            match="Acme",
            fields={"x": {"regex": "[unclosed", "type": "float"}},
        ),
    )["fields"]["x"]
...
def test_no_regex_in_dict_spec():
    f = extract_with_rule(ACME, make_rule(match="Acme", fields={"x": {"type": "float"}}))["fields"][
        "x"
    ]
...
def test_combine_captures():
    ...
    rule = make_rule(
        match="",
        fields={
            "name": {"regex": [r"First:\s*(\w+)", r"Last:\s*(\w+)"], "combine": " ", "type": "str"},
        },
    )
